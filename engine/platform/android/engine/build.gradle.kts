...
extensions.configure<LibraryExtension> {
    ...
    lint {
        ...
        // These pins match the oldest consumer (TV). Dependency upgrades
...
publishing {
    publications {
        register<MavenPublication>("release") {
            artifactId = "engine-android"
            afterEvaluate {
                from(components["release"])
            }
            pom {
                name.set("Engine for Android")
                description.set("Coroutine and Flow wrapper around the native streaming engine")
            }
        }
    }
    repositories {
