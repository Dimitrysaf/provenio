...
public enum UploadMode: Sendable, Equatable {
    case disabled
    case unlimited
    case limited(bytesPerSecond: UInt64)
}

public enum TorrentProfile: UInt32, Sendable, Equatable {
    case soft = 0
    case balanced = 1
    case fast = 2
}
...
public struct EngineConfiguration: Sendable, Equatable {
    ...
    public var torrentProfile: TorrentProfile
    ...
    public init(
        ...
        torrentProfile: TorrentProfile = .balanced,
        listenPort: UInt16 = 0,
        uploadMode: UploadMode = .unlimited,
...
extension EngineConfiguration {
    func validated() throws -> Self {
        ...
        if case let .limited(bytesPerSecond) = uploadMode, bytesPerSecond == 0 {
            throw EngineError(message: "limited upload mode requires a positive byte rate")
        }
        if let tlsCABundle {
            guard tlsCABundle.isFileURL else {
                throw EngineError(message: "TLS CA bundle must be a file URL")
            }
            ...
            guard values.isRegularFile == true else {
                throw EngineError(message: "TLS CA bundle must be a regular file")
            }
    ...
    private func validateDirectoryPath(_ url: URL, name: String) throws {
        guard url.isFileURL else {
            throw EngineError(message: "\(name) must be a file URL")
        }
...
func validateNativeString(_ value: String, name: String, limit: Int) throws {
    guard !value.isEmpty else {
        throw EngineError(message: "\(name) must not be empty")
    }
    guard !value.utf8.contains(0) else {
        throw EngineError(message: "\(name) must not contain NUL bytes")
    }
    guard value.lengthOfBytes(using: .utf8) <= limit else {
        throw EngineError(message: "\(name) exceeds the \(limit)-byte native limit")
    }
}
