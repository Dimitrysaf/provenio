...
public actor Engine {
    ...
    private var eventSubscribers: [UUID: AsyncStream<Event>.Continuation] = [:]
    private var statsSubscribers: [UUID: AsyncStream<EngineStats>.Continuation] = [:]
    private var latestStats = EngineStats()
    ...
    public nonisolated static var version: String {
        String(cString: engine_version_string())
    }

    public nonisolated static var protocolBackendVersion: String {
        String(cString: engine_protocol_backend_version())
    }
    ...
    public static func create(
        ...
        if Task.isCancelled {
            if let pointer = OpaquePointer(bitPattern: address) {
                engine_destroy(pointer)
            }
            throw CancellationError()
        }
        guard let pointer = OpaquePointer(bitPattern: address) else {
            throw EngineError(message: "native engine returned an empty handle")
        }
        let engine = Engine(handle: pointer)
        await engine.startPolling()
        return engine
    }

    public func eventStream() throws -> AsyncStream<Event> {
        try ensureOpen()
        let identifier = UUID()
    ...
    public func statsStream() throws -> AsyncStream<EngineStats> {
        try ensureOpen()
        let identifier = UUID()
        let (stream, continuation) = AsyncStream.makeStream(
            of: EngineStats.self,
    ...
    public func addMagnet(_ magnetURI: String) async throws -> String {
        ...
        let requestID = try magnetURI.withCString { magnetPointer in
            var request = engine_torrent_request()
            engine_torrent_request_init_sized(
                &request,
                UInt32(MemoryLayout<engine_torrent_request>.size)
            )
            request.magnet_uri = magnetPointer
            request.source_type = 0
            return try submit { handle, requestID in
                engine_add_torrent(handle, &request, &requestID)
            }
        }
        ...
        guard let torrentID = event.torrentID else {
            throw EngineError(message: "metadata-ready event omitted the torrent ID")
        }
        return torrentID
    }
    ...
    public func addTorrent(_ torrentData: Data) async throws -> String {
        guard !torrentData.isEmpty else {
            throw EngineError(message: "torrent data must not be empty")
        }
        guard torrentData.count <= maximumTorrentBytes else {
            throw EngineError(message: "torrent data exceeds the 4 MiB native limit")
        }
        let requestID = try torrentData.withUnsafeBytes { bytes in
            guard let baseAddress = bytes.baseAddress else {
                throw EngineError(message: "torrent data did not expose storage")
            }
            var request = engine_torrent_request()
            engine_torrent_request_init_sized(
                &request,
    ...
    public func files(for torrentID: String) throws -> [TorrentFile] {
        try validateTorrentID(torrentID)
        let handle = try openHandle()
        return try torrentID.withCString { torrentPointer in
            var count = 0
            try checkNativeStatus(engine_get_file_count(handle, torrentPointer, &count))
            var files: [TorrentFile] = []
            files.reserveCapacity(count)
            for position in 0..<count {
                var nativeFile = engine_file()
                engine_file_init_sized(
                    &nativeFile,
                    UInt32(MemoryLayout<engine_file>.size)
                )
                try checkNativeStatus(
                    engine_get_file(handle, torrentPointer, position, &nativeFile)
                )
    ...
    public func prepareStream(
        ...
        guard
            ...
            throw EngineError(message: "stream-prepared event omitted required fields")
        }
        return Stream(
            id: streamID,
            url: streamURL,
    ...
    public func stopStream(_ streamID: String) async throws {
        ...
        let requestID = try streamID.withCString { streamPointer in
            try submit { handle, requestID in
                engine_stop_stream(handle, streamPointer, &requestID)
            }
        }
    ...
    public func removeTorrent(_ torrentID: String) async throws {
        ...
        let requestID = try torrentID.withCString { torrentPointer in
            try submit { handle, requestID in
                engine_remove_torrent(handle, torrentPointer, &requestID)
            }
        }
    ...
    @discardableResult
    public func reclaimDiskCache(targetBytes: UInt64 = 0) async throws -> Event {
        let requestID = try submit { handle, requestID in
            engine_reclaim_disk_cache(handle, targetBytes, &requestID)
        }
    ...
    public func currentStats() throws -> EngineStats {
        try readStats()
    }

    public func currentStreamStats(_ streamID: String) throws -> StreamStats {
        try validateStreamID(streamID)
        let handle = try openHandle()
        return try streamID.withCString { streamPointer in
            var nativeStats = engine_stream_stats()
            engine_stream_stats_init_sized(
                &nativeStats,
                UInt32(MemoryLayout<engine_stream_stats>.size)
            )
            try checkNativeStatus(
                engine_get_stream_stats(handle, streamPointer, &nativeStats)
            )
            return StreamStats(
    ...
    public func preloadStream(
        ...
        guard minimumContiguousBytes <= maximumPreloadBytes else {
            throw EngineError(message: "minimum contiguous bytes exceeds 64 MiB")
        }
        ...
        guard
            ...
            throw EngineError(message: "stream URL is not the matching loopback endpoint")
        }
        ...
        guard
            ...
            throw EngineError(message: "loopback preload ended before its target")
        }
        for _ in 0..<40 {
    ...
    public func shutdown() async {
        pollingTask?.cancel()
        pollingTask = nil
        let closedError = EngineError(message: "Engine is closed")
    ...
    private func pollEventsAndStats() {
        ...
        while true {
            var nativeEvent = engine_event()
            engine_event_init_sized(
                &nativeEvent,
                UInt32(MemoryLayout<engine_event>.size)
            )
            let status = engine_poll_event(handle, &nativeEvent)
    ...
    private func process(_ event: Event) {
        ...
        if event.type == .torrentError {
            ...
            pending.continuation.resume(throwing: EngineError(
    ...
    private func submit(
        ...
        guard requestID != 0 else {
            throw EngineError(message: "native command returned an empty request ID")
        }
        return requestID
    }
    ...
    private func makeEvent(from nativeEvent: inout engine_event) throws -> Event {
        guard let type = EventType(rawValue: nativeEvent.type) else {
            throw EngineError(message: "unknown native event type \(nativeEvent.type)")
        }
        ...
        if let streamURLText {
            guard let parsedURL = URL(string: streamURLText) else {
                throw EngineError(message: "native event contained an invalid stream URL")
            }
            streamURL = parsedURL
        } else {
            streamURL = nil
    ...
    private func readStats() throws -> EngineStats {
        let handle = try openHandle()
        var stats = engine_stats()
        engine_stats_init_sized(
            &stats,
            UInt32(MemoryLayout<engine_stats>.size)
        )
        try checkNativeStatus(engine_get_stats(handle, &stats))
        return EngineStats(
    ...
    private func openHandle() throws -> OpaquePointer {
        try ensureOpen()
        guard let handle = OpaquePointer(bitPattern: handleAddress) else {
            throw EngineError(message: "Engine is closed")
        }
        return handle
    }
...
private func createNativeHandle(configuration: EngineConfiguration) throws -> UInt {
    guard engine_api_version() == 3 else {
        throw EngineError(message: "incompatible Engine C ABI")
    }
    ...
    return try dataPath.withCString { dataPointer in
        try cachePath.withCString { cachePointer in
            try withOptionalCString(caPath) { caPointer in
                var nativeConfiguration = engine_config()
                engine_config_init_sized(
                    &nativeConfiguration,
                    UInt32(MemoryLayout<engine_config>.size)
                )
                ...
                try checkNativeStatus(engine_create(&nativeConfiguration, &handle))
