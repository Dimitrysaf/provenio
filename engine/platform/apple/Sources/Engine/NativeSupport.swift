...
func nativeError(_ status: UInt32) -> EngineError {
    let pointer = engine_status_message(status)
...
func validateStreamID(_ streamID: String) throws {
    guard streamID.count == 64, streamID.allSatisfy(\.isHexDigit) else {
        throw EngineError(message: "stream ID must be a 64-character hexadecimal token")
    }
}
