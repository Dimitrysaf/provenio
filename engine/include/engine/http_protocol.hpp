#ifndef ENGINE_HTTP_PROTOCOL_HPP
#define ENGINE_HTTP_PROTOCOL_HPP
...
[[nodiscard]] ENGINE_CPP_API RequestParseResult parse_http_request_head(
    std::string_view request_head
);

[[nodiscard]] ENGINE_CPP_API StreamResponse build_stream_response(
    const HttpRequest& request,
    std::uint64_t resource_size,
    std::string_view content_type
);

[[nodiscard]] ENGINE_CPP_API std::string build_error_response(
    ResponseStatus status,
