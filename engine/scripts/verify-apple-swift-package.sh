...
if [[ ! -f "$apple_root/Engine.xcframework/Info.plist" ]]; then
...
swift package \
    ...
    >/dev/null
symbol_graph=$(find "$apple_root/.build" -path '*/symbolgraph/Engine.symbols.json' -print -quit)
if [[ -z "$symbol_graph" ]]; then
    echo "Swift package did not produce a Engine symbol graph" >&2
    exit 1
fi
if rg -q 'CEngine|engine_' "$symbol_graph"; then
