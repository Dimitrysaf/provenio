...
package_name="engine-windows-$target_architecture"
...
common_flags="-D_WIN32_WINNT=0x0A00 -DWINVER=0x0A00 -ffile-prefix-map=/source/platform/windows/.deps=/engine/dependencies -ffile-prefix-map=/source=/engine/source -ffunction-sections -fdata-sections -mguard=cf"
...
if ! cmake \
    ...
    -DFETCHCONTENT_SOURCE_DIR_BOOST="$boost_source" \
    -DFETCHCONTENT_SOURCE_DIR_LIBTORRENT="$libtorrent_source" \
...
mkdir -p \
    "$stage/bin" \
    "$stage/lib" \
    "$stage/include/engine" \
    "$stage/licenses" \
    "$stage/tools"
cp "$build_root/engine.dll" "$stage/bin/engine.dll"
cp "$build_root/libengine.dll.a" "$stage/lib/libengine.dll.a"
cp "$engine_root/cmake/engine-windows.def" "$stage/lib/engine.def"
/opt/llvm-mingw/bin/llvm-strip --strip-unneeded "$stage/bin/engine.dll"
cp "$engine_root/include/engine/engine.h" "$stage/include/engine/"
cp "$engine_root/include/engine/export.h" "$stage/include/engine/"
cp "$engine_root/LICENSE" "$stage/licenses/ENGINE-LICENSE.txt"
...
"/opt/llvm-mingw/bin/${target_triple}-clang" \
    ...
    -DENGINE_USING_SHARED \
    -I "$stage/include" \
    -L "$stage/lib" \
    -lengine \
    -mguard=cf \
    -Wl,--dynamicbase,--nxcompat,--high-entropy-va \
    -o "$stage/tools/engine_smoke.exe"
