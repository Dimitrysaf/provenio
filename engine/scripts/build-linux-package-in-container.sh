...
package_name="engine-linux-$target_architecture"
...
if ! cmake \
    ...
    -DCMAKE_C_FLAGS='-ffile-prefix-map=/source/platform/linux/.deps=/engine/dependencies -ffile-prefix-map=/source=/engine/source' \
    -DCMAKE_CXX_FLAGS='-ffile-prefix-map=/source/platform/linux/.deps=/engine/dependencies -ffile-prefix-map=/source=/engine/source' \
...
mkdir -p "$stage/lib" "$stage/include/engine" "$stage/licenses"
cp "$build_root/libengine.so.0.1.1" "$stage/lib/libengine.so.0.1.1"
strip --strip-unneeded "$stage/lib/libengine.so.0.1.1"
ln -s libengine.so.0.1.1 "$stage/lib/libengine.so.0"
ln -s libengine.so.0 "$stage/lib/libengine.so"
