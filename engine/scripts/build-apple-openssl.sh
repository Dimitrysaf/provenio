...
output_link="${TMPDIR:-/tmp}/apple-openssl-$link_key"
...
for index in "${!slice_names[@]}"; do
    ...
    marker="$install_directory/.openssl-${openssl_version}-v1"
...
create_universal_install() {
    ...
    local marker="$destination/.openssl-${openssl_version}-universal-v1"
    local first_marker="$installs/$first/.openssl-${openssl_version}-v1"
    local second_marker="$installs/$second/.openssl-${openssl_version}-v1"
