...
if [[ $# -ne 1 ]]; then
    echo "usage: $0 path/to/engine.aar" >&2
    exit 2
fi
...
if ! license=$(unzip -p "$temporary/classes.jar" META-INF/ENGINE_LICENSE.txt) \
    || [[ "$license" != "$(<"$engine_root/LICENSE")" ]]; then
    echo "AAR project license is missing or does not match" >&2
    exit 1
fi
if ! rg -q '^META-INF/ENGINE_THIRD_PARTY_NOTICES\.md$' <<< "$license_entries"; then
    echo "AAR does not package third-party notices" >&2
    exit 1
fi
...
for abi in armeabi-v7a arm64-v8a x86 x86_64; do
    library="$temporary/jni/$abi/libengine.so"
    if [[ ! -f "$library" ]]; then
        echo "missing $abi/libengine.so" >&2
        exit 1
    fi
    ...
    rg -q ' engine_create(@@ENGINE_0\.1)?$' <<< "$symbols"
    rg -q ' engine_reclaim_disk_cache(@@ENGINE_0\.1)?$' <<< "$symbols"
    rg -q ' Java_com_engine_internal_NativeBridge_nativeCreate(@@ENGINE_0\.1)?$' <<< "$symbols"
    unexpected_symbols=$(
        awk '{print $NF}' <<< "$symbols" \
            | rg -v '^(ENGINE_0\.1|engine_[A-Za-z0-9_]+(@@ENGINE_0\.1)?|Java_com_engine_internal_NativeBridge_[A-Za-z0-9_]+(@@ENGINE_0\.1)?)$' \
            || true
    )
