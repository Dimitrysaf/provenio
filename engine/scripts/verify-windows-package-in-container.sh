...
package="$temporary/engine-windows-$target_architecture"
library="$package/bin/engine.dll"
...
[[ -f "$package/lib/libengine.dll.a" ]] || fail "import library is missing"
cmp -s \
    "$package/lib/engine.def" \
    "$engine_root/cmake/engine-windows.def" || fail "export definition mismatch"
[[ -f "$package/tools/engine_smoke.exe" ]] || fail "smoke executable is missing"
cmp -s \
    "$package/include/engine/engine.h" \
    "$engine_root/include/engine/engine.h" || fail "C header mismatch"
cmp -s \
    "$package/include/engine/export.h" \
    "$engine_root/include/engine/export.h" || fail "export header mismatch"
...
license_pairs=(
    "$package/licenses/ENGINE-LICENSE.txt:$engine_root/LICENSE"
...
smoke_headers=$($readobj --file-headers "$package/tools/engine_smoke.exe")
grep -Fq "$expected_machine" <<< "$smoke_headers" || fail "smoke executable has wrong PE machine"
smoke_imports=$($readobj --coff-imports "$package/tools/engine_smoke.exe")
grep -Fq 'Name: engine.dll' <<< "$smoke_imports" || fail "consumer did not link the DLL import library"
