...
package="$temporary/engine-linux-$target_architecture"
library="$package/lib/libengine.so.0.1.1"
fail() {
...
[[ "$(readlink "$package/lib/libengine.so")" == libengine.so.0 ]] || \
    fail "unversioned library symlink is wrong"
[[ "$(readlink "$package/lib/libengine.so.0")" == libengine.so.0.1.1 ]] || \
...
readelf -d "$library" | grep -E 'SONAME.*\[libengine.so.0\]' >/dev/null || fail "wrong SONAME"
...
readelf --version-info "$library" | grep -E 'ENGINE_1\.0' >/dev/null || fail "ABI version node is missing"
