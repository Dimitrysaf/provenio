...
xcframework="${1:-$apple_root/Engine.xcframework}"
...
for slice in "$macos" "$ios" "$simulator"; do
    require_file "$slice/libCEngine.a"
    require_file "$slice/Headers/module.modulemap"
    require_file "$slice/Headers/engine/engine.h"
    require_file "$slice/Headers/engine/export.h"
    ...
    cmp -s \
        "$slice/Headers/engine/engine.h" \
        "$engine_root/include/engine/engine.h" || fail "C header mismatch in $slice"
    cmp -s \
        "$slice/Headers/engine/export.h" \
        "$engine_root/include/engine/export.h" || fail "export header mismatch in $slice"
done

[[ "$(find "$xcframework" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')" == 3 ]] || \
    fail "XCFramework does not contain exactly three platform slices"

assert_architectures "$macos/libCEngine.a" arm64 x86_64
assert_architectures "$ios/libCEngine.a" arm64
assert_architectures "$simulator/libCEngine.a" arm64 x86_64

temporary_directory=$(mktemp -d)
trap 'rm -rf "$temporary_directory"' EXIT
assert_build_version "$macos/libCEngine.a" arm64 MACOS 11.0 macos-arm64
assert_build_version "$ios/libCEngine.a" arm64 IOS 16.1 ios-arm64
assert_build_version \
    "$simulator/libCEngine.a" \
    arm64 \
    IOSSIMULATOR \
    16.1 \
    ios-simulator-arm64
...
api_symbols=(
    _engine_api_version
    _engine_version_string
    _engine_protocol_backend_version
    _engine_status_message
    _engine_create
    _engine_destroy
    _engine_add_torrent
    _engine_poll_event
    _engine_get_file_count
    _engine_get_file
    _engine_prepare_stream
    _engine_remove_torrent
    _engine_stop_stream
    _engine_get_stats
    _engine_get_stream_stats
    _engine_stream_stats_init_sized
    _engine_reclaim_disk_cache
)
for archive in \
    "$macos/libCEngine.a" \
    "$ios/libCEngine.a" \
    "$simulator/libCEngine.a"; do
...
if rg -q '/var/folders/|apple-openssl-' "$temporary_directory/archive-strings"; then
    fail "archives expose a temporary build path"
fi

license_pairs=(
    "$licenses/ENGINE-LICENSE.txt:$engine_root/LICENSE"
