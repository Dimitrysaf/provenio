...
output="$apple_root/Engine.xcframework"
...
reproducible_flags="-ffile-prefix-map=$compiler_engine_root=/engine/source -ffile-prefix-map=$compiler_dependency_root=/engine/dependencies"
...
configure_and_build() {
    ...
    if ! cmake --build "$build_directory" --parallel "${BUILD_JOBS:-8}" \
        >"$build_log" 2>&1; then
        cat "$build_log" >&2
        exit 1
    fi
...
merge_slice() {
    ...
    if ! xcrun libtool -static \
        -o "$destination/libCEngine.a" \
        "$build_directory/libengine.a" \
        "$build_directory/_deps/libtorrent-build/libtorrent-rasterbar.a" \
...
mkdir -p "$headers/engine"
cmake -E copy \
    "$engine_root/include/engine/engine.h" \
    "$headers/engine/engine.h"
cmake -E copy \
    "$engine_root/include/engine/export.h" \
    "$headers/engine/export.h"
cmake -E copy \
...
mkdir -p "$licenses"
cmake -E copy "$engine_root/LICENSE" "$licenses/ENGINE-LICENSE.txt"
...
xcodebuild -create-xcframework \
    -library "$build_root/combined/macos/libCEngine.a" \
    -headers "$headers" \
    -library "$build_root/combined/ios/libCEngine.a" \
    -headers "$headers" \
    -library "$build_root/combined/ios-simulator/libCEngine.a" \
    -headers "$headers" \
    -output "$output"
