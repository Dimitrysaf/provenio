...
runtime_prefix='C:/engine/openssl'
...
cp /source/cmake/openssl-windows-targets.conf \
    "$source_root/Configurations/99-windows.conf"

compiler_version=$("/opt/llvm-mingw/bin/${target_triple}-clang" --version | awk 'NR == 1 { print $4 }')
marker="$install_directory/.openssl-${openssl_version}-windows-v1"
...
if ! (
    ...
    perl "$source_root/Configure" \
        ...
        --openssldir='C:/ProgramData/OpenSSL' \
        --libdir=lib \
        -D_WIN32_WINNT=0x0A00 \
        -ffile-prefix-map=/source=/engine/source \
        no-asm \
        no-shared \
        no-tests \
        no-apps \
