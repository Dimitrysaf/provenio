...
TEST("loopback listener serves a bounded partial response") {
    const std::string content = "abcdefghij";
    http::LoopbackServer server(
        0,
        [&](const http::HttpRequest& request,
            ...
            EXPECT_TRUE(writer(std::span<const char>(response.headers)));
            if (response.include_body) {
                EXPECT_TRUE(response.body_range.has_value());
                ...
                EXPECT_TRUE(writer(std::span<const char>(content).subspan(start, length)));
            }
        }
    );
    ...
    EXPECT_TRUE(response.find("HTTP/1.1 206 Partial Content\r\n") == 0);
    EXPECT_TRUE(response.find("Content-Range: bytes 2-5/10\r\n") != std::string::npos);
    EXPECT_TRUE(response.ends_with("cdef"));
}

TEST("loopback listener cancels a handler when its client disconnects") {
    ...
    EXPECT_TRUE(handler_started.load());
    ...
    EXPECT_TRUE(handler_cancelled.load());
    EXPECT_TRUE(
    ...
    EXPECT_EQ(server.active_request_count(), std::uint32_t(0));
}

TEST("loopback listener rejects malformed requests before dispatch") {
    std::atomic_int handler_calls = 0;
    http::LoopbackServer server(
        0,
        [&](const http::HttpRequest&,
            const http::LoopbackServer::Writer&,
            const std::stop_token) {
            ++handler_calls;
        },
        2,
...
TEST("loopback listener cancels an incomplete active request on stop") {
    http::LoopbackServer server(
        0,
        [](const http::HttpRequest&,
           const http::LoopbackServer::Writer&,
