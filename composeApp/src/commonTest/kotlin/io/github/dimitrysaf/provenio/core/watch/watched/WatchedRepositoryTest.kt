...
class WatchedRepositoryTest {
    ...
    @Test
    fun successfulTrackerPush_doesNotAcknowledgeFailedAccountPush() {
        ...
        assertFalse(
            shouldAcknowledgeAccountWatchedPush(
    ...
    @Test
    fun successfulAccountPush_acknowledgesAccountDirtyState() {
        val outcome = WatchedPushOutcome(accountSyncSucceeded = true)
    ...
    fun watchedItemsForSource_keepsProviderSnapshotsIsolated() {
        val accountItem = watchedItem(id = "provenio", markedAtEpochMs = 1_000L)
        ...
        assertEquals(
            ...
            watchedItemsForSource(
                source = WatchProgressSource.ACCOUNT_SYNC,
                accountItems = listOf(accountItem),
                providerItems = mapOf(
    ...
    fun successfulTrackerPush_waitsForRemoteSnapshotAcknowledgement() {
        ...
        assertFalse(shouldAcknowledgeAccountWatchedPush(WatchProgressSource.TRAKT, outcome))
    }
    ...
    @Test
    fun replacingTraktSnapshot_doesNotOverwriteAccountSnapshot() {
        val accountItem = watchedItem(id = "provenio", markedAtEpochMs = 1_000L)
        ...
        val accountItems = mutableMapOf("provenio" to accountItem)
        ...
        assertEquals(mapOf("provenio" to accountItem), accountItems)
        assertEquals(
    ...
    @Test
    fun effectiveWatchedSource_fallsBackToAccountWhenTraktIsUnavailable() {
        assertEquals(
