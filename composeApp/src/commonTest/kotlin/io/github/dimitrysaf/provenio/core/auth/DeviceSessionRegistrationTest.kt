...
class DeviceSessionRegistrationTest {
    ...
    fun buildsOfficialClientRegistrationPayload() {
        val params = buildDeviceRegistrationParams(
            installationId = "provenio-mobile-installation",
            clientVersion = "1.2.3",
        ...
        assertEquals("provenio-mobile-installation", params.getValue("p_installation_id").toString().trim('"'))
        assertEquals("Provenio Mobile", params.getValue("p_client_name").toString().trim('"'))
