...
import provenio.composeapp.generated.resources.streams_plugin_repository_fallback
...
internal data class PluginProviderGroup(
    ...
    /**
     * A repository's row borrows the icon of the first scraper that has one: a repository has no
     * icon of its own, and its scrapers all come from the same place.
     */
    val addonLogo: String?
        get() = scrapers.firstNotNullOfOrNull { it.logo?.takeIf { logo -> logo.isNotBlank() } }
}
