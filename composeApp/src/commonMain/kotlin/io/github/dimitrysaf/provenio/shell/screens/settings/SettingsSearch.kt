...
internal fun settingsSearchEntries(
    ...
    listOf(
        PlaybackSearchRow("provenio-license", stringResource(Res.string.settings_licenses_attributions_upstream_title), stringResource(Res.string.settings_licenses_attributions_upstream_license)),
...
internal fun LazyListScope.settingsSearchRootContent(
    ...
    if (!isTablet && (showSearchField || query.isNotBlank())) {
...
/**
 * The settings search bar: the app's search field with the settings placeholder, on the search
 * bar's own container.
 *
 * The root's results are the page itself, so there is no divider under the bar. The container is
 * what sets it apart from what follows, which is why this is the one search in the app that has
 * one. m3.material.io/components/search/specs
 */
@Composable
...
/** Half the bar's 56dp height: `SearchBarDefaults.InputField` is fully rounded. */
private val SearchFieldCorner = 28.dp
